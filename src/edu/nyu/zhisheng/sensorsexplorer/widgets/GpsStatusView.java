...
public class GpsStatusView extends SquareView {
	 ...
	 */
	private void drawSat(Canvas canvas, int prn, float azimuth,
			float elevation, float snr, boolean used) {

		float r = (90 - elevation) * mW * 0.9f / 200;
		float x = (float) (r * Math.sin(azimuth * Math.PI / 180));
		float y = (float) -(r * Math.cos(azimuth * Math.PI / 180));

		canvas.drawCircle(x, y, snr * snrScale, used ? activePaint
				: inactivePaint);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		// Log.d("GpsStatusView", String.format("Drawing on a %dx%d canvas", w,
		// h));
		...
		canvas.drawTextOnPath(
				((Activity) getContext()).getString(R.string.value_N),
				labelPathN, 0, -labelPaint.descent(), labelPaint);

		canvas.drawTextOnPath(
				((Activity) getContext()).getString(R.string.value_S),
				labelPathS, 0, -labelPaint.descent(), labelPaint);

		canvas.drawTextOnPath(
				((Activity) getContext()).getString(R.string.value_E),
				labelPathE, 0, -labelPaint.descent(), labelPaint);

		canvas.drawTextOnPath(
				((Activity) getContext()).getString(R.string.value_W),
		...
		if (mSats != null) {
			for (GpsSatellite sat : mSats) {
				...
				if ((azDelta < 5) && (eleDelta < 5)) {
					Log.d("GpsStatusView",
							String.format(
									"Satellite %d, snr=%f, azimuth=%f, elevation=%f, almanac=%b, ephemeris=%b, used=%b",
									sat.getPrn(), sat.getSnr(),
									sat.getAzimuth(), sat.getElevation(),
									sat.hasAlmanac(), sat.hasEphemeris(),
									sat.usedInFix()));
				}
				drawSat(canvas, sat.getPrn(), sat.getAzimuth(),
						sat.getElevation(), sat.getSnr(), sat.usedInFix());
			}
		}
	}
	...
	public void refreshGeometries() {
		...
		float offsetX = mW * 0.0275f
				* (float) Math.cos(Math.toRadians(mRotation + 90));
		float offsetY = mW * 0.0275f
				* (float) Math.sin(Math.toRadians(mRotation + 90));
	...
	public void setYaw(float yaw) {
		mYaw = yaw;
		mRotation = mYaw
				+ zeroYaw[((Activity) getContext()).getWindowManager()
