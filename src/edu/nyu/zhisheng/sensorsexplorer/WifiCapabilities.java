...
public abstract class WifiCapabilities {
	 ...
	 * 
	 * This method detects captive portals (also known as walled gardens), which
	 * redirect Web traffic to a sign-in page as long as the user has not
	 * provided any credentials. It does so by connecting to a particular Web
	 * address, which will respond with HTTP status code 204 (no content) and an
	 * empty result body. If a different response (such as a document or
	 * redirection) is obtained, it will assume the presence of a captive
	 * portal.
	 * 
	 ...
	 * the main UI thread. Consider instead creating an {@link AsyncTask} around
	 * it.
	 * 
	...
	public static int getNetworkConnectivity() {
		...
		Log.d(WifiCapabilities.class.getSimpleName(), "Checking " + mUrl
				+ " to see if we're behind a captive portal");
		try {
			...
			Log.d(WifiCapabilities.class.getSimpleName(),
					"Probably not a portal: exception " + e);
